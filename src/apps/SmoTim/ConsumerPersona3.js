...
const ConsumerPersona = () => {
  ...
  // Handle streaming submit (GET request with SSE)
  const handleSubmit = async () => {
    ...
    try {
      // Build GET query string from form data
      const brandName = encodeURIComponent(formData.brandName);
      const brandDescription = encodeURIComponent(formData.brandDescription);
      const url = `/api/generate-persona3?brandName=${brandName}&brandDescription=${brandDescription}`;

      // Fetch via GET (SSE style)
      ...
      while (true) {
        ...
        // SSE typically separates messages with double newlines:
        ...
        for (let i = 0; i < lines.length - 1; i++) {
          ...
          if (line.startsWith("data: ")) {
            ...
            if (!dataStr) continue; // skip empty lines
            ...
            try {
              ...
              // If the chunk includes an imageUrl, treat that as final
              if (data.imageUrl) {
                personaData = { ...personaData, imageUrl: data.imageUrl };
              ...
              } else if (data.type === "final" && data.persona) {
                // Or if the server sends a "final" event with the entire persona
                personaData = data.persona;
                setPersona(personaData);
                setHasSubmitted(true);
                localStorage.setItem("consumerPersonaSubmitted", "true");
              } else {
                // Merge partial data with what we have so far
